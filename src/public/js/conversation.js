document.addEventListener('DOMContentLoaded',() => {
  ...
  const source = document.querySelector('#source-language'), target = document.querySelector('#target-language')
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      const result=await api('/api/processing/transcribe',{method:'POST',body:JSON.stringify({target_language:target.value,audio:{size:blob.size,type:blob.type}})})
      transcript=result.data.transcript; translated=result.data.translated_transcript; renderPanel('original',transcript);renderPanel('translated',translated);setStatus('Completed');summarySection.classList.remove('hidden')
