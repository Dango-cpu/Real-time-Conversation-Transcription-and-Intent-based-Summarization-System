...
from model import CnnBiLstmCtc
...
def run_epoch(model, loader, criterion, optimizer, device, text):
    ...
    wer = sum(word_error_rate(ref, hyp) for ref, hyp in zip(references, hypotheses)) / max(1, len(references))
    return total_loss / max(1, len(loader)), wer
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_wer = run_epoch(model, train_loader, criterion, optimizer, device, text)
        with torch.no_grad():
            valid_loss, valid_wer = run_epoch(model, valid_loader, criterion, None, device, text)
        print(
            f"epoch={epoch} train_loss={train_loss:.4f} train_wer={train_wer:.4f} "
            f"valid_loss={valid_loss:.4f} valid_wer={valid_wer:.4f}"
        )
